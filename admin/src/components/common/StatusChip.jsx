...
const StatusChip = ({ status, label, size = 'small', sx, ...rest }) => {
  ...
  return (
    <Chip
      size={size}
      label={label || (key ? key[0].toUpperCase() + key.slice(1) : '—')}
      {...rest}
      sx={{
        color: tone.color,
        ...
        ...(rest.onClick ? { cursor: 'pointer', '&:hover': { filter: 'brightness(0.95)' } } : null),
        ...sx,
      }}
    />
  );
