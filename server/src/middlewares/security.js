 ...
 *
 * express-rate-limit's default store is a Map in this process, which is the
 * right answer for the single API instance this app runs: counting in memory
 * costs nothing, cannot fail the request, and needs no infrastructure to keep
 * alive. The caveat to remember if a second instance is ever added is that
 * each one counts separately, so a caller gets the configured limit *per
 * instance* — '20 sign-in attempts per 15 minutes' quietly becomes 40. At that
 * point the counters have to move to a store both instances share; until then
 * they do not.
 */
